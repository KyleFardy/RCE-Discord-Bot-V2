  ...
  async execute(data, rce, client) {
    try {
      ...
      await client.functions.log(
        ...
        `\x1b[38;5;208m[${
          server.identifier
        }]\x1b[0m \x1b[32;1m[KILL]\x1b[0m \x1b[32;1m${
          killer.type === 'npc' ? 'A Scientist' : killer.name
        }\x1b[0m Killed \x1b[38;5;214m${
          victim.type === 'npc' ? 'A Scientist' : victim.name
        }\x1b[0m!`
      );
      ...
      const kd_ratio = calculate_kd_ratio(
        killer_kill_count,
        killer_death_count
      );
      ...
      await client.functions.log(
        'error',
        `Error Handling Player Kill Event: ${err.message}`
      );
    }
  },
};
...
async function handle_npc_kill(client, server, killer, current_server) {
  await client.player_stats.add_points(
    server,
    killer.name,
    current_server.npc_kill_points
  );
  try {
    const killer_name = await get_member_name(
      client,
      killer.name,
      current_server
    );
    await client.functions.send_embed(
      ...
      `${server.identifier} - New NPC Kill`,
      '',
      [
    ...
    await client.rce.servers.command(
      server.identifier,
      `say <color=green><b>[NPC KILL]</b></color> <color=green><b>${killer.name}</b></color> Killed A <color=red><b>Scientist</b></color>!`
    );
  } catch (err) {
    await client.functions.log(
      'error',
...
async function handle_entity_death(client, server, victim, current_server) {
  await client.player_stats.remove_points(
    server,
    victim.name,
    current_server.npc_death_points
  );
  try {
    const victim_name = await get_member_name(
      client,
      victim.name,
      current_server
    );
    await client.functions.send_embed(
      client,
      current_server.kill_feeds_channel_id,
      `${server.identifier} - New NPC Death`,
      '',
      [
    ...
    await client.rce.servers.command(
      server.identifier,
      `say <color=green><b>[NPC DEATH]</b></color> <color=green><b>${victim.name}</b></color> was killed by a <color=red><b>Scientist</b></color>!`
    );
  } catch (err) {
    await client.functions.log(
      'error',
...
async function get_death_count(client, player_name, condition) {
  return await client.functions.get_count(
    client,
    `SELECT COUNT(*) as count FROM kills WHERE display_name = ? AND ${condition}`,
    [player_name]
  );
}
...
async function update_player_stats(
  client,
  server,
  killer,
  victim,
  ...
  await Promise.all([
    ...
    client.player_stats.add_points(
      server,
      killer.name,
      current_server.player_kill_points
    ),
    client.player_stats.remove_points(
      server,
      victim.name,
      current_server.player_death_points
    ),
