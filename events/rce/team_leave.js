...
async function handle_team_disbanded(
  client,
  server_id,
  owner_name,
  team_id,
  ...
  await client.functions.log(
    'info',
    `\x1b[38;5;208m[${server_id}]\x1b[0m \x1b[32;1m[TEAM DISBANDED] \x1b[0m${owner_name}\x1b[32;1mHas Deleted Their Team \x1b[32;1m(${team_id})!\x1b[0m`
  );
...
async function handle_team_left(
  client,
  server_id,
  member_name,
  owner,
  ...
  await client.functions.log(
    'info',
    `\x1b[38;5;208m[${server_id}]\x1b[0m \x1b[32;1m[TEAM LEFT] \x1b[0m${member_name}\x1b[32;1mHas Left \x1b[0m${owner}'s Team \x1b[32;1m(${team_id})!\x1b[0m`
  );

  const owner_name = get_member_name(client, owner, server);
  await send_team_left_log(
    client,
    member_name,
    owner_name,
    server_id,
...
async function send_team_deleted_log(
  client,
  member_name,
  server_id,
  team_id,
...
async function send_team_left_log(
  client,
  member_name,
  owner_name,
  server_id,
