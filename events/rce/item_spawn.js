...
module.exports = {
  ...
  async execute(data, rce, client) {
    ...
    await client.functions.log(
      'info',
      format_item_spawn_message(server, ign, quantity, item)
    );
    ...
    try {
      ...
      await client.functions.send_embed(
        client,
        current_server.item_spawning_channel_id,
      ...
      await client.functions.log(
        'error',
        `Failed to send Item Spawned embed for ${item} to ${current_server.item_spawning_channel_id}:`,
        error
      );
    }
  },
