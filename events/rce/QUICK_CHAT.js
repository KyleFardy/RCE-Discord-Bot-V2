...
module.exports = {
    ...
    async execute(data, rce, client) {
        ...
        await handle_quick_chat_message(client, ign, server, message, current_server);
    }
};

// Helper function to log quick chat messages
async function log_quick_chat_message(client, server_id, ign, message) {
    await client.functions.log("info", `\x1b[38;5;208m[${server_id}]\x1b[0m \x1b[32;1m[QUICK CHAT]\x1b[0m ${ign} Sent \x1b[38;5;208m${message}\x1b[0m!`);
}

// Helper function to handle quick chat messages
async function handle_quick_chat_message(client, ign, server, message, current_server) {
    switch (message) {
        ...
        case QuickChat.NEED_MetalFragments:
            await client.functions.handle_vip_kit(client, ign, current_server, server);
            break;
        case QuickChat.QUESTIONS_WantToTrade:
            if (client.functions.is_empty(current_server.outpost)){
                await client.rce.servers.command(server.identifier, `say <color=green><b>[TELEPORT]</b></color> <color=green><b>Outpost</b></color> Teleport Has <color=red><b>Not</b></color> Been Setup Yet!`);
                return;
            }
            await client.functions.handle_teleport(client, ign, current_server.outpost, "Outpost", server);
            break;
        case QuickChat.QUESTIONS_CouldYouHelpMe:
            if (client.functions.is_empty(current_server.bandit)) {
                await client.rce.servers.command(server.identifier, `say <color=green><b>[TELEPORT]</b></color> <color=green><b>Bandit Camp</b></color> Teleport Has <color=red><b>Not</b></color> Been Setup Yet!`);
                return;
            }
            await client.functions.handle_teleport(client, ign, current_server.bandit, "Bandit Camp", server);
            break;
    }
}
