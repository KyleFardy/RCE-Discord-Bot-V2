...
module.exports = {
    ...
    async execute(data, rce, client) {
        const { ign, owner, team_id, server: { identifier } } = data; // Destructure data
        ...
        await send_team_creation_log(client, member_name, identifier, team_id, current_server);
    }
};
...
function get_member_name(client, owner, server) {
    return client.guilds.cache.get(server.guild_id)?.members.cache.find(member => member.nickname === owner || member.user.username === owner)?.toString() || owner;
}

// Helper function to send a team creation log message
async function send_team_creation_log(client, member_name, server_id, team_id, server) {
