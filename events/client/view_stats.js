  ...
  async execute(interaction, client) {
    ...
    if (interaction.customId.startsWith('view_stats_')) {
      ...
      try {
        ...
        const stats = await get_player_stats(
          client,
          player_info.display_name,
          server_identifier,
          interaction
        );
        const embed = await create_stats_embed(
          interaction,
          player_info,
          stats,
          server_identifier
...
async function get_player_stats(client, display_name, server, interaction) {
  const current_server = await client.functions.get_server_discord_identifier(
    client,
...
async function create_stats_embed(interaction, player_info, stats, server) {
  const current_server =
    await interaction.client.functions.get_server_discord_identifier(
      interaction.client,
      interaction.guild.id,
      server
    );
  ...
  const last_kill = stats.last_kill
    ? format_player_link(stats.last_kill.victim, interaction.client)
    : 'Not Killed Anyone';
  const last_killer = stats.last_death
    ? format_player_link(stats.last_death.display_name, interaction.client)
    : 'Not Been Killed';
  const worst_enemy = stats.worst_enemy
    ? format_player_link(stats.worst_enemy.display_name, interaction.client) +
      ` (Deaths: *${stats.worst_enemy.KillCount}*)`
    : 'Not Found';
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: 'Currency',
        value: `🪙 ${player_info.currency} (Use In <#${current_server.shop_channel_id}>)`,
        inline: true,
      },
