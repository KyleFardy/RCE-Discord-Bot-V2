...
class rce_bot {
  constructor() {
    ...
    this.client = new Client({
      ...
      allowedMentions: {
        // Control who gets mentioned in responses
  ...
  async load_items() {
    try {
      ...
      this.client.functions.log(
        'error',
        `[ITEMS] Failed To Load Items: ${err.message}`
      );
    }
  }
  ...
  async init_database() {
    try {
      ...
      this.client.functions.log(
        'debug',
        '\x1b[32;1m[DATABASE]\x1b[0m Connection Established!'
      );
    } catch (err) {
      this.client.functions.log(
        'error',
        '\x1b[32;1m[DATABASE]\x1b[0m Connection Failed!',
        err
      );
    }
  }
  ...
  async load_rce_events() {
    ...
    for (const file of eventFiles) {
      try {
        ...
        this.client.functions.log(
          'debug',
          `\x1b[32;1m[RCE EVENT]\x1b[0m ${this.client.functions.get_event_name(
            event.name
          )} Event Loaded!`
        );
        ...
        if (event.once) {
          this.client.rce.events.once(event.name, (...args) =>
            event.execute(...args, this.client.rce.events, this.client)
          );
        } else {
          this.client.rce.events.on(event.name, (...args) =>
            event.execute(...args, this.client.rce.events, this.client)
          );
        }
      } catch (error) {
        this.client.functions.log(
          'error',
          `\x1b[32;1m[RCE EVENT]\x1b[0m Failed To Load Event ${file}: ${error.message}`
        );
      }
    }
  }
  ...
  async load_events() {
    this.client.functions.log(
      'debug',
      '\x1b[34;1m[BOT]\x1b[0m Loading Events...'
    );
    const eventFiles = fs
    ...
    for (const file of eventFiles) {
      try {
        ...
        this.client.functions.log(
          'debug',
          `\x1b[32;1m[DISCORD EVENT]\x1b[0m ${this.client.functions.get_event_name(
            event.name
          )} Event Loaded!`
        );
        ...
        this.client.functions.log(
          'error',
          `\x1b[32;1m[DISCORD EVENT]\x1b[0m Failed To Load Event ${file}: ${error.message}`
        );
      }
    }
  }
  ...
  async fetch_servers() {
    try {
      const [rows] = await this.client.database_connection.execute(
        'SELECT * FROM servers'
      ); // Get all servers
      ...
      this.client.functions.log(
        'debug',
        `\x1b[34;1m[BOT]\x1b[0m ${this.client.servers.length} Servers Successfully Fetched From The Database!`
      );
    } catch (error) {
      this.client.functions.log(
        'error',
        '\x1b[34;1m[BOT]\x1b[0m Error Fetching Servers: ' + error.message
      );
    }
  }
  ...
  async start() {
    this.client.functions.log(
      'info',
      '\x1b[34;1m[BOT]\x1b[0m Starting The Bot...'
    );
    ...
    try {
      ...
      this.client.functions.log(
        'error',
        '\x1b[34;1m[BOT]\x1b[0m Error During Start: ' + error.message
      );
    }
  }
  ...
  async register_commands() {
    ...
    try {
      ...
      this.client.functions.log(
        'debug',
        '\x1b[32;1m[COMMANDS]\x1b[0m Registered Commands With Discord API!'
      );
    } catch (error) {
      this.client.functions.log(
        'error',
        '\x1b[32;1m[COMMANDS]\x1b[0m Failed To Register Commands: ' +
          error.message
      );
    }
  }
